	...
	public static boolean doFiledownload(String saveFilename, String originalFilename, String pathname, HttpServletResponse resp) {
		...
		try {
			originalFilename=new String(
					originalFilename.getBytes("euc-kr"), "8859_1"); //클라이언트가 파일을 다운 받을 때 파일명 8859_1 형식으로 변환해서 다운 받을 수 있게, originalFilename 명을 쏴줘야함
			...
			// 파일명은 헤더에, 이름먼저 쏴줘야함 originalFilename으로 
	...
	public static String doFilerename(String pathname, String filename) {
    	...
    	String s = String.format(
    			"%1$tY%1$tm%1$td%1$tH%1$tM%1$tS", 
				 Calendar.getInstance()); //년월일 시분초+nanoTime
    	s += System.nanoTime();
    	s += fileExt;
